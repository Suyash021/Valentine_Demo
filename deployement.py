    ...
    <style>
        ...
        * {
        ...
        .section-title {
            ...
            font-size: 3rem;
            color: white;
            text-shadow: 2px 2px 4px rgba(255, 68, 132, 0.5);
            margin-bottom: 40px;
        }
        ...
        .photo-card {
            ...
            border-radius: 40px;
            padding: 25px;
            box-shadow: 0 15px 40px rgba(255, 68, 132, 0.2);
            border: 3px solid white;
        ...
        .photo-card:hover {
            ...
            box-shadow: 0 25px 50px rgba(255, 68, 132, 0.4);
        }
        
        .photo-frame {
            width: 100%;
            aspect-ratio: 1;
            background: linear-gradient(135deg, #ffe6e9, #ffd4db);
            border-radius: 30px;
            overflow: hidden;
...
<body>
    <div class="container">
        ...
        <div class="photo-section">
            ...
            <div class="photo-grid" id="photoGallery">
                ...
                <!-- Photo 3 -->
                <div class="photo-card">
                    <div class="photo-frame" id="frame3">
                        <div class="photo-placeholder">
                            <div class="placeholder-icon">📸</div>
                            <div class="placeholder-text">Your photo here</div>
                        </div>
                ...
                <!-- Photo 4 -->
                <div class="photo-card">
                    <div class="photo-frame" id="frame4">
                        <div class="photo-placeholder">
                            <div class="placeholder-icon">📸</div>
                            <div class="placeholder-text">Your photo here</div>
                        </div>
        ...
        <div class="upload-section">
            <h3 class="upload-title">📸 Add Your Photos</h3>
            
            <div class="upload-grid">
                <!-- Upload Box 1 -->
                <div class="upload-box" onclick="document.getElementById('file1').click()">
                    <div class="upload-icon">📸</div>
                    <div class="upload-label">First Date</div>
                    <div class="upload-hint">Click to upload</div>
                </div>
                <input type="file" id="file1" class="file-input" accept="image/*" onchange="uploadPhoto(1, this)">
                
                <!-- Upload Box 2 -->
                <div class="upload-box" onclick="document.getElementById('file2').click()">
                    <div class="upload-icon">💍</div>
                    <div class="upload-label">Engagement</div>
                    <div class="upload-hint">Click to upload</div>
                </div>
                <input type="file" id="file2" class="file-input" accept="image/*" onchange="uploadPhoto(2, this)">
                
                <!-- Upload Box 3 -->
                <div class="upload-box" onclick="document.getElementById('file3').click()">
                    <div class="upload-icon">🌸</div>
                    <div class="upload-label">Favorite Moment</div>
                    <div class="upload-hint">Click to upload</div>
                </div>
                <input type="file" id="file3" class="file-input" accept="image/*" onchange="uploadPhoto(3, this)">
                
                <!-- Upload Box 4 -->
                <div class="upload-box" onclick="document.getElementById('file4').click()">
                    <div class="upload-icon">💕</div>
                    <div class="upload-label">Recent Memory</div>
                    <div class="upload-hint">Click to upload</div>
                </div>
                <input type="file" id="file4" class="file-input" accept="image/*" onchange="uploadPhoto(4, this)">
            </div>
            
            <button class="save-btn" onclick="saveToLocalStorage()">💾 Save All Photos</button>
            <button class="save-btn" onclick="loadFromLocalStorage()" style="background: linear-gradient(45deg, #ff4567, #ff6b81);">📂 Load Saved Photos</button>
        </div>
        ...
        <div class="footer">
            ...
            <p style="font-size: 1.5rem;">14 February 2026</p>
            <p style="font-size: 2.5rem; margin-top: 20px;">❤️</p>
        </div>
    </div>
    ...
    <script>
        // Store photos
        let photos = {
            1: localStorage.getItem('rekha_photo_1') || null,
            2: localStorage.getItem('rekha_photo_2') || null,
            3: localStorage.getItem('rekha_photo_3') || null,
            4: localStorage.getItem('rekha_photo_4') || null
        };
        ...
        function uploadPhoto(photoNumber, input) {
            ...
            if (file) {
                ...
                reader.onload = function(e) {
                    // Display photo
                    displayPhoto(photoNumber, e.target.result);
                    
                    // Save to variable
                    photos[photoNumber] = e.target.result;
                    
                    // Show success
                    showMessage(`Photo ${photoNumber} uploaded! ✨`);
                };
        ...
        function displayPhoto(photoNumber, imageData) {
            const frame = document.getElementById(`frame${photoNumber}`);
        ...
        function saveToLocalStorage() {
            ...
            for (let i = 1; i <= 4; i++) {
                if (photos[i]) {
                    localStorage.setItem(`rekha_photo_${i}`, photos[i]);
                    savedCount++;
                }
            }
            
            if (savedCount > 0) {
                showMessage(`✅ ${savedCount} photos saved! They will stay here forever!`);
            } else {
                showMessage('No photos to save. Upload some photos first!');
            }
        }
        ...
        function loadFromLocalStorage() {
            ...
            for (let i = 1; i <= 4; i++) {
                const savedPhoto = localStorage.getItem(`rekha_photo_${i}`);
                if (savedPhoto) {
                    displayPhoto(i, savedPhoto);
                    photos[i] = savedPhoto;
                    loadedCount++;
                }
            }
            ...
            if (loadedCount > 0) {
                showMessage(`📂 Loaded ${loadedCount} saved photos!`);
            } else {
                showMessage('No saved photos found. Upload some photos first!');
            }
        }
        ...
        function removePhoto(photoNumber) {
            ...
            // Remove from storage
            photos[photoNumber] = null;
            localStorage.removeItem(`rekha_photo_${photoNumber}`);
            
            showMessage(`Photo ${photoNumber} removed`);
        }
        ...
        // Add click handlers for photo cards to remove photos
